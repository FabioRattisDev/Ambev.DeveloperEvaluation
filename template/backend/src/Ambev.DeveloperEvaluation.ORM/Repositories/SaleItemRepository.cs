...
public class SaleItemRepository : ISaleItemRepository
{
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>A list of sale items if found, null otherwise</returns>
    public async Task<List<SaleItem>?> GetByIdAsync(Guid saleId, CancellationToken cancellationToken = default)
    {
    ...
    {
        var saleItem = await _context.SaleItems.FirstOrDefaultAsync(si => si.Id == id, cancellationToken);
        if (saleItem == null)
            return false;
